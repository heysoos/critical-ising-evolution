...
from compute_heatcapacity_generational_recorded_function import save_compute_heat_capacity
import os
import numpy as np
...
def compute_plot_heat_capacity(sim_name, generation_list, cores, settings, recorded):
    ...
    if recorded:
        ...
        for gen_str in generation_list:
            for i in range(beta_num):
                save_compute_heat_capacity(sim_name, i, gen_str)

    else:
        print('Sina says: Bad heat capacity settings!!')
...
if __name__ == '__main__':
    ...
    #              ]
    # sim_names = ['sim-20211020-165202_parallel_beta_linspace_rec_c250_5_sims']
    folder_name = 'sim-20211020-165202_parallel_beta_linspace_rec_c250_5_sims'
    sim_names = all_sim_names_in_parallel_folder(folder_name)
    folder_name = 'sim-20211026-155738_parallel_beta_linspace_rec_c250_20_sims_hardtask'
    sim_names = sim_names + all_sim_names_in_parallel_folder(folder_name)
    for sim_name in sim_names:
        cores = 15
        generation_list = np.arange(0, 4000, 250) #[0]
        settings = load_settings(sim_name)
